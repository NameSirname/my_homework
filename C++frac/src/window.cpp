...
void Window::drawImage(bool redraw)
{
	int k, stride = fractal.width;
	
	if(redraw) build(&fractal);

	for(k=0;k<deph->value();k++) palette.push_back(color(k).rgb());
...
QColor Window::color(int k)
{
	double K = (double)k/deph->value();
	return QColor::fromHslF(fabs(sin(K*sqrt(k))),1,0.25+sqrt(K)*3/4);
}
