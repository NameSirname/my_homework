...
###
        ...
        f_file = open("fractal_parallel.h","w")
    ...
    return z*z+c;
        ...
        f_file.close()
        p = sub.Popen(["gcc","-Wall","-Wextra","-fopenmp","fractal_parallel.c",
                       "-lm","-o","a1.o"],
                  stdout=sub.PIPE)
        p.communicate()
    ...
    def Enter(self):
        if self.func.text()!= self.prev_func:
            self.prev_func = self.func.text()
...
###                      
    ...
    def bitmap(self):
        ...
        start = time.time()
        p = sub.Popen(["./a1.o"], stdout=sub.PIPE)
        p.communicate()
        print("{0:.2f} sec".format(time.time()-start))

        out = open("out.txt","r")
        B = np.array([[int(e) for e in line.split()] for line in out.read().split('\n')[:-1]])
        out.close()
