...
def ZOOM(self,position):
    Zoom = QPushButton("Lupa'n'Pupa", self)
    Zoom.clicked.connect(self.Zoom)
    self.grid.addWidget(Zoom,position,0,1,self.row)
    return None
...
def BOUNDS(self,position,rows, DefaultBounds = ['-1.5','1.5','0']):
    self.minX = QLineEdit(DefaultBounds[0],self)
    self.grid.addWidget(self.minX,position,0,1,rows)

    self.maxX = QLineEdit(DefaultBounds[1],self)
    self.grid.addWidget(self.maxX,position+1,0,1,rows)
    
    self.offset = QLineEdit(DefaultBounds[2],self)
    self.grid.addWidget(self.offset,position+1,rows,1,self.row-rows)

    self.offset.l = QLabel('offset')
    self.offset.l.setStyleSheet("color: rgba(255,255,255)")
    self.grid.addWidget(self.offset.l,position,rows,1,self.row-rows)
    return None
