...
class QGraphicsView(QGraphicsView):
    ...
    def Generate(self):
        ...
        self.p.point.setPos(self.p.y*side+(0.5-(z%2)*(-1)**(z//2%2))*side//2,
                          self.p.x*side+(0.5+(z%2-1)*(-1)**(z//2%2))*side//2)
        self.s.addItem(self.p.point)

        self.s.items()[pos(self.p.x,self.p.y,h,w)].setBrush(self.p.c_unit)
        return None
...
def repaintL(self,x,y,h,w,i,k=1):
    if k:
        if self.Lab[x][y][i] == 1:
            self.pen = QPen(self.c_def,3)
        else:
            self.pen = QPen(self.c_wall,3)
    else:
        if self.Lab[x][y][i] == 0:
            self.pen = QPen(self.c_def,3)
        else:
