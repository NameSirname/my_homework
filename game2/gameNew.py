...
##        self.move(0,0)
    ...
    def ResizeLab(self):
        if not self.state:
            ...
            self.coords[0].setRange(0,self.hL-1)
            self.coords[1].setRange(0,self.wL-1)
            self.coords[3].setRange(0,self.wL-1)
...
##            self.scene.clear()
    ...
    def Load(self):
        if not self.state:
            ...
            if filename!='':
                ...
                self.wL,self.hL,Y,x,y,z = map(int,file.readline().split())
                self.LabHeight.setValue(self.hL)
                self.LabWidth.setValue(self.wL)
                self.ResizeLab()
                i=0
                for e in [x,y,z,Y]:
                    self.coords[i].setValue(e)
                    i+=1
    ...
    def Save(self):
        ...
        if filename!='':
            ...
            file.write(str(self.wL)+' '+str(self.hL)+' '+str(self.exit[1])+' '+
                       ' '.join(list(map(str,self.xyz)))+'\n')
            for i in range(self.hL+1):
    ...
    def Start(self):
        if self.state:
            x,y,z,Y = (self.coords[i].value() for i in range(4))
            self.state = not self.state
            self.log.setStyleSheet("color: black;")
            self.log.setText("#")
            self.moves = 0
            self.proc = [0]*(self.N+2)
            self.scene.items()[pos(self.xyz[0],self.xyz[1],self.hL,self.wL)].setBrush(self.brush)
            repaintL(self,0,self.exit[1],self.hL,self.wL,0)
            self.xyz = [x,y,z]
##            print(self.exit[1],Y,self.Lab[0][Y])
            self.exit = [0,Y,0]
            repaintL(self,0,Y,self.hL,self.wL,0)
