...
###
        self.CMap2 = QComboBox(self)
        self.CMap2.addItems(A)
        self.CMap2.addItems(my_cmaps.A)
        #self.CMap2.addItems(['my_colormap#'])
                            
        self.CMap2.setCurrentText('plasma')
        grid.addWidget(self.CMap2,5,0,1,2)
        self.CMap2.activated.connect(self.CMap_act)

        self.CMap2.l = QLabel('cmap2')
        self.CMap2.l.setStyleSheet("color: rgba(255,255,255)")
        grid.addWidget(self.CMap2.l,5,2,1,-1)
...
class Mplot(FCanvas):
    def __init__(self, parent,B=[]):
        ...
        self.colorMap2 = parent.CMap2.currentText()
...
F = lambda z:'''+ self.parent.func.text(),globals())
        ...
        ax.imshow(abs(self.B), cmap = self.colorMap)
##        ax.imshow(self.B.imag, cmap = self.colorMap2 ,alpha=self.alpha)
        
        ax.contour(self.B.imag, self.num_l, cmap = self.colorMap2 ,
                   alpha=self.alpha, linewidths = [self.width_l]*self.num_l)        
